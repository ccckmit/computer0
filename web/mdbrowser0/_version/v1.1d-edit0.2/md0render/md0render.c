...
void draw_edit_mode(App* app) {
    ...
    if (app->edit_type == 1) {
        ...
        int y = top_margin + 5;
        int len = strlen(app->edit_buf);
        int cursor_screen_x = left_margin + 5;
        int cursor_screen_y = y;
        
        int logical_line = 0;
        int logical_pos_in_line = 0;
        for (int j = 0; j < app->edit_cursor && j < len; j++) {
            if (app->edit_buf[j] == '\n') {
                logical_line++;
                logical_pos_in_line = 0;
            } else {
                logical_pos_in_line++;
            }
        }
        ...
        while (i < len) {
            ...
            while (i < len && app->edit_buf[i] != '\n') {
                ...
                if (screen_line == logical_line && pos_in_line == logical_pos_in_line) {
                    cursor_screen_x = x;
                    cursor_screen_y = y;
                }
            ...
            if (screen_line == logical_line && i >= app->edit_cursor) {
                cursor_screen_x = x;
                cursor_screen_y = y;
            }
            ...
            if (app->edit_buf[i] == '\n') {
                i++;
                if (screen_line == logical_line && pos_in_line == logical_pos_in_line) {
                    cursor_screen_x = left_margin + 5;
                    cursor_screen_y = y - line_height;
                }
            }
        ...
        SDL_SetRenderDrawColor(app->ren, 0, 0, 0, 255);
        SDL_RenderDrawLine(app->ren, cursor_screen_x, cursor_screen_y + 2, cursor_screen_x, cursor_screen_y + font_height);
    }
    ...
    if (e->type == SDL_KEYDOWN) {
        if (e->key.keysym.sym == SDLK_ESCAPE) {
            ...
            if (unicode >= 32 && unicode < 127) {
                ...
                int cur_len = strlen(app->edit_buf);
                if (app->edit_cursor < cur_len) {
                    memmove(app->edit_buf + app->edit_cursor + u8len, 
                            app->edit_buf + app->edit_cursor, 
                            cur_len - app->edit_cursor + 1);
                }
                if (app->edit_cursor + u8len < MAX_EDIT_BUF) {
                    memcpy(app->edit_buf + app->edit_cursor, utf8, u8len);
                    app->edit_cursor += u8len;
                }
            }
        }
    }
